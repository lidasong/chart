...
import im from './im'
import { IAxises } from 'src/interfaces/Chart'
...
function formatTime(value: string | number | Date, formal: string = '') {
  try {
...
function getChartType(chartType: any, config: any) {
  return isDuplex(config.column || config.row)
    ? 'duplex'
    : chartType || 'bar'
}
...
function isDiscrete(pill: Pill|undefined) {
  ...
  return interpretation === 'discrete' || interpretation === undefined && isDimension(pill)
}

function isContinuous(pill: Pill) {
  const interpretation = im.get(pill, 'interpretation')
  return interpretation === 'continuous' || interpretation === undefined && isMeasure(pill)
}

export {
  formatTime, unique, cloneDeep, getFieldName, getFieldAlias,
  isTranpose, isDimension, isMeasure, equal, noop,
  getChartType, isPie, isDuplex, isDiscrete, isContinuous
}
