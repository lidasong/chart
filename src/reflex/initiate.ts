...
import { DEFAULT_CONFIG, LEGEND_TOP_BOTTOM, LEGEND_COMMON_STYLE } from './consts'
import { cloneDeep } from '../utils'
import { isArray, isObject } from '../utils/type'
import { ILegend } from 'src/interfaces/Chart'
      ...
      if (typeof config[configKey] === 'object' && config[configKey] !== null) {
        ...
        if (isArray(config[configKey])) {
          wrapedConfig = config[configKey].map((item: any, index: number) => {
            if (isObject(item)) {
              const partSettingItem = isArray(partSetting[configKey]) ? im.get(partSetting[configKey], String(index)) : partSetting[configKey]
