import {
  getFieldName, getFieldAlias,
  isDiscrete, isTranpose,
  getChartType, isPie
} from '../utils'
...
import { Pill } from 'src/interfaces/Pill'
import { IGeom, ILegend, IConfig } from 'src/interfaces/Chart'
import {
  setFormat, getFormatter, setColorConfig, setGeomShape, setGeomAdjust, setGeomRealType,
  setLabelConfig, setSizeConfig, getFilter, setGeomToolTip, setChartPadding, setChartFilter,
  setAxisScale, setRealLegendPos
} from './tool'
class Reflex {
  config: any
  setCoordConfig() {
    ...
    const radiusConfig = im.get(geom, 'radiusConfig', .8)
  ...
  setAxisConfig() {
    ...
    axisConfig = row.concat(column).filter((pill: Pill) => !!getFieldName(pill)).reduce((axisConfig: {}, pill: Pill) => {
      const name = getFieldName(pill)
      let config = im.get(axisConfig, name, {})
  ...
  setTooltipConfig() {
    ...
    const showTitle =
      geom.every((childGeom: IGeom) => {
        ...
        const geomFieldNames = [color].concat(label, detail).filter(isDiscrete).map(item => getFieldName(item))
        const isPerspective = dimensions.map(item => getFieldName(item)).some((fieldName: string) => {
    ...
    scale = {
      ...
      ...(isArray(geom) ? geom : [geom]).reduce((result: any, geom: IGeom) => {
        const { color, size, shape, label, detail } = geom
          ;[color, size, shape, label, detail].filter((pill: any) => !!getFieldName(pill)).forEach((item: any) => {
      ...
      ...([].concat(row, column)).filter((pill: Pill) => !!getFieldName(pill)).reduce(
        (
          result: any,
          pill: Pill
        ) => {
  ...
  setLegendConfig() {
    ...
    let mergedLegend = geom
      .map((childGeom: IGeom, index: number) => {
    ...
    mergedLegend = mergedLegend.concat(legend.slice(mergedLegend.length))
      .filter((item: any) => !!item.name)
    let padding = im.get(chart, 'padding').slice()
