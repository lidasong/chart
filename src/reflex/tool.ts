import {
  formatTime, unique, formatWithThousandPlace, getFieldName, isDiscrete, isContinuous,
  isTranpose, isDuplex, equal, isPie
} from '../utils'
import im from '../utils/im'
import { isArray, isFunction } from '../utils/type'
import { G2 } from 'bizcharts'
import { IFormatter, IFilter } from '../interfaces/Formatter'
import { MARGIN_RATIO, CHART_MAPPING, CHART_PADDING_MAP, LEGEND_POSITION } from './consts'
import { IGeom, IConfig } from 'src/interfaces/Chart'
const {
  colors: baseColors,
  colors_16,
  colors_24,
...
export function setColorConfig(config: any) {
  ...
  geom = geom.map((geom: any = {}) => {
    ...
    colorConfig = colorConfig || (isContinuousColor ? DEFAULT_CONTINUOUS_COLOR : '')
    const { colorConfigMap = {} } = options
    const colorMap = im.get(colorConfigMap, colorFiledName)
    let colors = []
    if (colorFiledName && colorMap) {
      const colorCategories = unique(data.map((item: any) => item[colorFiledName]))
...
export function setChartFilter(config: IConfig): IConfig {
  const { chart = {} } = config
  const { filter = {} } = chart
  const chartFilters = Object.keys(filter).reduce((result: any, key: string) => {
...
export function setGeomToolTip(geom: IGeom, { row = {}, column = {} }: any, index: number) {
    ...
    (geom = {
      ...
      tooltip:
        ...
        (chartType === 'map'
          ...
          .filter(item => !!item)
          .filter(item => !isColumnMulti || (isColumnMulti && item !== rowName))
